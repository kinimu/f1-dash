...
import { translateCountry, translateSessionKind, translateString, translateDay } from "@/lib/translations";
			...
			<div className="flex items-center justify-between border-b border-zinc-800 pb-2">
				<div className="flex items-center gap-2">
					<div className="flex items-center gap-2">
						...
						<p className="text-2xl">{translateCountry(round.countryName)}</p>
					</div>
					{round.name === nextName && (
						<>
							{utc().isBetween(utc(round.start), utc(round.end)) ? (
								<p className="text-indigo-500">{translateString("Current")}</p>
							) : (
								<p className="text-indigo-500">{translateString("Up Next")}</p>
							)}
						</>
					)}
					{round.over && <p className="text-red-500">{translateString("Over")}</p>}
				</div>
			...
			<div className="grid grid-cols-3 gap-8 pt-2">
				{groupSessionByDay(round.sessions).map((day, i) => (
					<div className="flex flex-col" key={`round.day.${i}`}>
						<p className="my-3 text-xl text-white">{translateDay(utc(day.date).local().format("dddd"))}</p>
						...
						<div className="grid grid-rows-2 gap-2">
							{day.sessions.map((session, j) => (
								...
								>
									<p className="w-28 overflow-hidden text-ellipsis whitespace-nowrap sm:w-auto">{translateSessionKind(session.kind)}</p>
